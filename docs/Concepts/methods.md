...
- **built-in methods**, which are provided by 4D and that can be called in your code. Built-in methods include:
	- Commands and functions of the 4D API, such as `ALERT` or `Current date`.
	 - Methods attached to collections or ORDA objects, such as `collection.orderBy()` or `entity.save()`.
...
The `New formula` or `New formula from string` methods allow you to create native formula objects that you can encapsulate in object properties. It allows you to implement custom object methods. 
